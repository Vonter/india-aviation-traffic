...
# Read the CSV file
data <- "../aggregated/domestic/carrier.csv"
df <- read_csv(data)
df <- df %>%
  filter(Type == "ScheduledDomestic") %>%
  select(Airline, Year, Month, `Passenger Number`)
...
selected_airlines <- airline_passenger_sums %>%
  filter(total_passengers >= 10000000) %>%
  pull(Airline)
df <- df %>%
  filter(Airline %in% selected_airlines, Airline != "Total Domestic")

# Group by year and month
df <- df %>%
  mutate(Date = as.Date(paste(Year, sprintf("%s", Month), "01", sep = "-"))) %>%
  select(Airline, Date, `Passenger Number`) %>%
  distinct()
...
brand_colors <- c(
  ...
  "Akasa Air" = "#000080",
  # Navy blue
  "AirAsia India" = "#FFCC00" # Golden yellow
)
...
p <- ggplot(df,
  ...
  ggtitle("Domestic air ridership in India (by carrier)") +
  ...
  # Add labels
  labs(x = "Year", y = "Monthly Passengers (Domestic)", color = "Airline")
